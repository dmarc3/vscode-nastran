## [NLPARM](https://help.hexagonmi.com/bundle/MSC_Nastran_2022.4/page/Nastran_Combined_Book/qrg/bulkno/TOC.NLPARM.xhtml) - Parameters for Nonlinear Static Analysis Control
...
4. The stiffness update strategy is selected in the KMETHOD field.
     ...
     • If the FNT option is selected, the program will use the full Newton iteration method for which the stiffness matrix will be updated at every iteration. FNT option is available for SOL 400 only. For SOL 106, please use “KMETHOD=ITER and KSTEP=1” instead. In comparison with the PFNT method, the defaults for FNT are EPSU=0.01, EPSW=0.01 and MAXLS=4. See
Remark 

     • If the PFNT option is selected, the program will use the Pure Full Newton iteration method. The PFNT method is the same as the FNT method except that the defaults for PFNT method are internally set as EPSU=-0.01, EPSW=-0.01, and MAXLS=0. The PFNT method is available for SOL 400 only. See Remark 
