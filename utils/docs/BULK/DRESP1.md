...
19. For RTYPE = LAMA, EIGN or FREQ, the response approximation used for optimization can be individually selected using the ATTB field when APRCOD = 1 is being used.
     ...
     1. If the ID of a DRESP1 with ATTB character input is referenced by a DRESP2 using the "DRESP2" flag, the results of the mathematical function are used to evaluate the DRESP2.
...
35. For monitor point responses (RTYPE = STMONP1, STMOND1, MONPNT3 AEMONP1 or AEMOND1) the ATTA field specifies the components to be extracted.  These can be any subset of the integers 1 through 6 that appear on the monitor quantity with the NAME provided in the PTYPE field.  All of these responses can be invoked in a static aeroelastic (ANALYSIS=SAERO) subcase. STMOND1 and MONPNT3 can be invoked from a static  (ANALYSIS=STAT) subcase. The responses are not available in a dynamic response or normal modes subcase.  The response types have the following meaning:
     - STMONP1 – A structural MONPNT1
     - STMOND1 – A structural MONDSP1
     - MONPNT3 – A MONPNT3
     - AEMONP1 – An aerodynamic MONPNT1
     - AEMOND1 – An aerodynamic MONDSP1
...
43. For the WMPID (weight as a function of material and property ID), the following conventions apply:
     - ATTA is required, ATTB and ATTi are optional.
     - ATTB specifies the superelement with ATTB=0 indicating the residual, >0, indicates superelement ATTB and ATTB = ALL or blank means all superelements)
     -  The ATTi fields designate the property ID's of interest. If ATTi is specified, the associated property type (e.g., PSHELL) must be specified by the PTYPE input of field 5 on the DRESP1 entry.
     -  If there is no ATTi, all material with MID=ATTA will be used to generate the response.
     - If there are multiple ATTi, a single scalar weight is calculated that is the sum of the all the properties
     - Materials that are supported with this entry are: MAT1,MAT2, MAT3, MAT8, MAT9 and MAT10
     - Properties that are supported are: PSHELL, PROD, PBEAM, PTUBE, PSHEAR, PBAR, PSOLID, PBEND,PCOMP, PCOMPG, PBARL, PBEAML, PBRSECT and PBMSECT.
     - PARAM WTMASS is not applied to this response.
44. Certain restrictions apply for RTYPE=WMPID
     - The density of the material referenced on the ATTA field cannot also be designed using a DVMRELx entry
