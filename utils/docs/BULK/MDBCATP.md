...
**Note:** The parameters defined in MDBCATP are applied only for BCONATCT=AUTO. Their default values are defined only when BCONTACT=AUTO is applied. All the parameters in BCPARA, BCONPRP and BCONPRG entries may be used in MDBCATP directly. For the detailed list of these parameters, please refer to BCPARA, BCONPRP and BCONPRG
